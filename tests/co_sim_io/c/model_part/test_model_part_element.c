...
int main()
{
    ...
    CoSimIO_Element elem_tri = CoSimIO_ModelPart_CreateNewElement(
        model_part,
        2,
        CoSimIO_Triangle2D3,
        connectivity);
    ...
    CoSimIO_Element elem_line = CoSimIO_ModelPart_CreateNewElement(
        model_part,
        5,
        CoSimIO_Line2D2,
        connectivity);
    ...
    CoSimIO_Element elem_point = CoSimIO_ModelPart_CreateNewElement(
        model_part,
        8,
        CoSimIO_Point2D,
        connectivity);
    ...
    for (int i=0; i<CoSimIO_Element_NumberOfNodes(elem_tri); ++i) {
        COSIMIO_CHECK_INT_EQUAL(i+1, CoSimIO_Node_Id(CoSimIO_Element_GetNodeByIndex(elem_tri, i)));
    }

    for (int i=0; i<CoSimIO_Element_NumberOfNodes(elem_line); ++i) {
    ...
    CoSimIO_Element elem_tri_by_index = CoSimIO_ModelPart_GetElementByIndex(model_part, 0);
    CoSimIO_Element elem_line_by_index = CoSimIO_ModelPart_GetElementByIndex(model_part, 1);
    CoSimIO_Element elem_point_by_index = CoSimIO_ModelPart_GetElementByIndex(model_part, 2);

    CoSimIO_Element elem_tri_by_id = CoSimIO_ModelPart_GetElementById(model_part, 2);
    CoSimIO_Element elem_line_by_id = CoSimIO_ModelPart_GetElementById(model_part, 5);
    CoSimIO_Element elem_point_by_id = CoSimIO_ModelPart_GetElementById(model_part, 8);
