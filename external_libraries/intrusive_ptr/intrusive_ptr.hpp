...
public:
    ...
    template<class U>
    intrusive_ptr( intrusive_ptr<U> const & rhs, typename CoSimIO::detail::sp_enable_if_convertible<U,T>::type = CoSimIO::detail::sp_empty() )
        : px( rhs.get() )
    {
    ...
    template<class U>
    intrusive_ptr(intrusive_ptr<U> && rhs, typename CoSimIO::detail::sp_enable_if_convertible<U,T>::type = CoSimIO::detail::sp_empty())
        : px( rhs.px )
    {
        rhs.px = 0;
    }
...
template< class T > std::size_t hash_value( CoSimIO::intrusive_ptr<T> const & p ) noexcept
{
